...
//! `luminance` crate gives you all the abstraction over OpenGL, but it doesn’t give you a way to
//! create an OpenGL context. This is due to the fact that creating and managing OpenGL contexts is
//! tightly related to the type of application. Typical PC applications might need something like
//! **GLFW** or **glutin** whilst others will directly bind to **X11** or **Windows API**. Several
//! crate – `luminance-*` – exist to solve that problem. They all provide a different implementation
//! for a simple need: opening an OpenGL context, opening a window and manage events. In theory, you
//! could even have a `luminance-gtk` or `luminance-qt` to embed `luminance` in surfaces in those
//! libraries.
//!
//! # What’s included
//!
//! This crate gives several important types that all backends must use. Among them, you’ll find:
//!
//! - `WindowDim`: abstraction over the dimension of a window and its mode (windowed, fullscreen).
//! - `WindowOpt`: an opaque type giving access to hints to customize the window integration.
//!
//! The `Device` trait must be implemented by a backend so that an application is completely
//! agnostic of the managing of the backend. This trait defines several basic methods that will help
//! you to:
//!
...
pub trait Device: Sized {
  ...
  /// # Note
  ///
  /// This associated type will disappean on long-term because it’s likely to be replaced by either
  /// [ATC](https://github.com/rust-lang/rust/issues/44265) or [conservative `impl Trait` expanded](https://github.com/rust-lang/rust/issues/42183).
